...
arquivos_data = ['simulacao_principal_data.py']

def executar_arquivo_data(arquivo):
    try:
        num_round = [20]
        total_clients = [20]
        modelos = ['DNN','CNN']
...
arquivos_dados = ['ferramentas/conta.py', 'ferramentas/XGB_CLASSIFIER.py']


max_threads = 1
for i in range(70):
    with concurrent.futures.ThreadPoolExecutor(max_threads) as executor:
        resultados = list(executor.map(executar_arquivo_data, arquivos_data))

processar_dados(arquivos_dados)
