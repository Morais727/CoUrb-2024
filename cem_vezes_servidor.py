...
class Timming(fl.server.strategy.FedAvg):
    ...
    def __init__(
        ...
        *,
        ...
        if os.path.exists(minmax_mnist_dnn_path) and os.path.exists(modelo_mnist_dnn_path):
            ...
            # self.loaded_model_dnn = tf.keras.models.load_model(modelo_mnist_dnn_path)
        ...
        if os.path.exists(minmax_cifar10_cnn_path) and os.path.exists(modelo_cifar10_cnn_path):
            ...
            # self.loaded_model_cnn = tf.keras.models.load_model(modelo_cifar10_cnn_path)
        else:
        ...
        for client, fit_res in results:
            ...
            if server_round > 1 and ataque != 'NORMAL' :
                ...
                if situacao == prev[0]:
                    ...
                    nome_arquivo = f"TESTES/cem_vezes/{fit_res.metrics['iid_niid']}/GRADIENTES/{modelo}/{ataque}/gradiente_{iid}_{server_round}.npy"
    ...
    def aggregate_evaluate(
        ...
        for client,eval_res in results:            
            nome_arquivo = f"TESTES/cem_vezes/{eval_res.metrics['iid_niid']}/LOG_EVALUATE/{eval_res.metrics['ataque']}_{eval_res.metrics['dataset']}_{eval_res.metrics['modelo']}_{eval_res.metrics['porcentagem_ataque']}_{eval_res.metrics['alpha_dirichlet']}_{eval_res.metrics['noise_gaussiano']}_{eval_res.metrics['round_inicio']}.csv"
            os.makedirs(os.path.dirname(nome_arquivo), exist_ok=True)   
            with open(nome_arquivo,'a') as file:          
                file.write(f"\n{server_round},{client.cid},{eval_res.metrics['accuracy']},{eval_res.loss}")
        
        arquivo_verifica_acertos = f"TESTES/cem_vezes/{eval_res.metrics['iid_niid']}/LOG_ACERTOS/{eval_res.metrics['ataque']}_{eval_res.metrics['dataset']}_{eval_res.metrics['modelo']}_{eval_res.metrics['porcentagem_ataque']}_{eval_res.metrics['alpha_dirichlet']}_{eval_res.metrics['noise_gaussiano']}_{eval_res.metrics['round_inicio']}.csv"        
